...
class HoverEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # --- START NEW CODE ---
        # Buffer to stre indices of envs that completed a success in the current episode.
        self.completed_target_idx = torch.tensor([], device=gs.device, dtype=gs.tc_int)
    ...
    def step(self, actions):
        ...
        #saving indices of successful drones
        self.completed_target_idx = resample_indices
    ...
    def _reward_target_completion(self):
        # This reward gives a one-time bonus when the drone reaches the target.
        completion_rew = torch.zeros((self.num_envs,), device=gs.device, dtype=gs.tc_float)
        if len(self.completed_target_idx) > 0:
            completion_rew[self.completed_target_idx] = 1.0
        return completion_rew
